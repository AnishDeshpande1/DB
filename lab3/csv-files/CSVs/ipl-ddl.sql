...
create table match
	(match_id			int, 
	 ...
	 toss_winner		int,
	 match_winner		int,
	 toss_name			varchar(10) check (toss_name in ('field','bat')),
	 win_type			varchar(10) check (win_type in ('wickets','runs') or win_type is null), 
	 man_of_match		int,
	 win_margin			int,
...
create table ball_by_ball
	(match_id		int,
	 ...
	 over_id		int,
	 ball_id		int,
	 runs_scored	int check (0<=runs_scored and runs_scored<=6),
	 extra_runs		int,
	 out_type		varchar(50) check (out_type in ('caught','caught and bowled','bowled','stumped','retired hurt', 'keeper catch', 'lbw', 'run out', 'hit wicket', 'NULL') or out_type is null),
	 striker 		int,
	 non_striker	int,
	 bowler			int, 
